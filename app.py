...
levels = {
    ...
    "8. 拖延：晚一點再買": {"objection": "保險很重要，但我現在不想決定，晚一點再買。", "hint": "提示：詢問要買便宜還是貴的？", "logic": "1.年紀成本 2.健康狀況不可逆"}
}

with st.sidebar:
    st.title(f"👤 {st.session_state.user_data['display_name']}")
    st.metric("🏆 最佳戰績 EXP", st.session_state.user_data['exp'])
    if st.button("登出系統"):
        st.session_state.logged_in = False
        st.session_state.messages = []
        st.rerun()
...
if user_input := st.chat_input("請輸入你的元捷實戰話術..."):
    try:
        ...
        3. 【實戰講評】：指出優缺點，並提供一句示範金句。
        4. 【元捷進修建議】：以熱血的語氣，引導學員針對表現弱點，回到「元捷教育訓練系統」查看相關教材或課程，強調系統化練習的重要性。
        """
